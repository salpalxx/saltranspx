...
for(tis in tislist)
  ...
  #print(dim(fres)[1])
  #print the number of trans-px gene pairs with FDR < 0.10
  fres_fdr0.10 <- dplyr::filter(fres,FDR<0.10)
  transpairs <- dim(fres_fdr0.10)[1]
  ...
  #get number of unique trans-px gene pairs with FDR<0.10
  replicated <- dim(dplyr::filter(unique_genematch,FDR < 0.10))[1]
  ...
  summary(lm(data=genematch,transpx.log10P~Westra.log10P))
    ...
    topmatches <- dplyr::filter(unique_genematch,FDR<0.10) %>% mutate(tis="FHSobs_v_GTEx" %&% tis %&% "pred")
  #print(topmatches)
    write.table(topmatches, "/home/sally/Westra_rep_trans-diffChrs_FHSobs_v_GTEx" %&% tis %&% "pred_FDR0.10_",quote=F,row.names=F)
  
  results <- list(cohort, tis, transpairs, replicated)
  
  #, trans-pairs, overlap)
  fwrite(results, file="/home/sally/newFDR.10table.txt", append=TRUE, sep=" ")
