...
for(tis in tislist)
  ...
  #print(topmatches)
    write.table(topmatches, "/home/sally/Westra_rep_trans-diffChrs_FHSobs_v_GTEx" %&% tis %&% "pred_FDR0.05_",quote=F,row.names=F)
  
  results <- list(cohort, tis, transpairs, replicated)
  
  #, trans-pairs, overlap)
  fwrite(results, file="/home/sally/newFDR.05table.txt", append=TRUE, sep=" ")
